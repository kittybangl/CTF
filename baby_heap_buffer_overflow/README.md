...
你可以手工放一个假的 flag 文件，验证 exp 的命令执行链是否通。

```bash
cd baby_heap_buffer_overflow
printf 'flag{this_is_a_fake_local_flag}\n' > flag
python3 solve.py --cmd 'cat ./flag'
```

预期会看到：

```text
flag{this_is_a_fake_local_flag}
```
