...
public class Schnorr_EUFCMA_Reduction extends A_Schnorr_EUFCMA_Reduction {
    ...
    public SchnorrSignature<BigInteger> sign(String message) {
        ...
        IGroupElement R = this.gen.power(s).multiply(this.x.power(minus_c));
        // BigInteger tmp = hash(message, R);
        Sites.put(new Pair<String,IGroupElement>(message, R), c);
        SchnorrSignature<BigInteger> ans = new SchnorrSignature<BigInteger>(c, s);
        return ans;
    }
    ...
    public BigInteger run(I_DLog_Challenger<IGroupElement> challenger) {
        ...
        int seed = randNum.nextInt(10000);
