...
def RSA_pad_encrypt(e: int, N: int, ptxt: bytes) -> bytes:
    ...
    print("m", m)
    return m, pow(m, e, N).to_bytes(RSA_KEYLEN // 8, 'big')
...
class SuboptimalRSAServer(CommandServer):
    ...
    def handle_query(self, msg):
        m, enc_flag = RSA_pad_encrypt(self.key.e, self.key.n, self.flag.encode())
        self.send_message({"flag": enc_flag.hex(), "m": m})
