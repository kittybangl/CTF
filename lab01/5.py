from itertools import cycle
from binascii import unhexlify
def xor(X, Y):
    return bytes(x ^ y for (x, y) in zip(X, Y))
alphas = "abcdefghijklmnopqrstuvwxyz{},"
CHALLENGE_CIPHERTEXT = ""
print(len(CHALLENGE_CIPHERTEXT))
tmp = [CHALLENGE_CIPHERTEXT[i:i+240] for i in range(0, len(CHALLENGE_CIPHERTEXT), 240)]
ans = ""
for t in tmp:
    print(t)
test = [t[:2] for t in tmp]
# for i in range(len(test)):
#     for j in range(len(test)):
#         if i == j:
#             continue
#         x = xor(unhexlify(test[i]), unhexlify(test[j]))
#         for k in alpha:
#             try:
#                 tmp = xor(k.encode(), x).decode()
#                 if tmp.isalpha() or tmp == '{' or tmp == '}' or tmp == ',':
#                     print(k, x, tmp)
#             except:
#                 continue
            # print(i, j, x.hex())

for alpha in alphas:
    flag = True
    for i in range(len(test)):
        # if flag == False:
        #     break
        if i == 0:
            continue
        x0_xor_xi = xor(unhexlify(test[0]), unhexlify(test[i]))
        # print(test[0], test[i], alpha)
        try:
            xi = xor(x0_xor_xi, alpha.encode()).decode()
            print(i, xi, flag)
            if xi.isalpha() == False and xi != '{' and xi != '}' and xi != ',':
                flag = False
                continue
        except:
            flag = False
            continue
    if flag == True:
        print(alpha)

print(test)
